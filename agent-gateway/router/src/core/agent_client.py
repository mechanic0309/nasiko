...
class AgentClient:
    ...
    async def send_request(self, agent_url: str, request: UserRequest, files: List[Tuple[str, Tuple[str, bytes, str]]], token: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            error_msg = f"HTTP error from agent {agent_url}: {e.response.status_code} - {e.response.text}"
            logger.error(error_msg)
            raise AgentClientError(error_msg) from e
            
        except httpx.RequestError as e:
            error_msg = f"Request error to agent {agent_url}: {str(e)}"
            logger.error(error_msg)
            raise AgentClientError(error_msg) from e
            
        except Exception as e:
            error_msg = f"Unexpected error communicating with agent {agent_url}: {str(e)}"
            logger.error(error_msg)
            raise AgentClientError(error_msg) from e
    
    def _construct_payload(self, request: UserRequest, files: List[Tuple[str, Tuple[str, bytes, str]]], agent_url: str) -> Dict[str, Any]:
        """
