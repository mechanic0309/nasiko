...
from router.src.core import (
    ...
    VectorStoreService, VectorStoreError,
...
class RouterOrchestrator:
    ...
    async def process_request(
        ...
        try:
            if request.route is None:
                ...
                async for response in self._handle_direct_route(request, files, request.route, token):
                    yield response
    ...
    async def _handle_route_selection(
        ...
        try:
            ...
            if not agent_cards:
                yield self._router_response("No agents available in registry", "", False, "")
                return
        ...
        try:
            ...
            logger.info(f"Prepared {len(truncated_agent_cards)} agent cards for routing")
        ...
        try:
            ...
            yield self._router_response("Determining the best agent to serve the user's query...")
        ...
        # Step 4: Route selection using AI
        try:
            shortlisted_agents, router_output = router(
                request.query, truncated_agent_cards, vectorstore
            )
        ...
        # Step 5: Get agent URL and send request
        try:
    ...
    async def _handle_direct_route(
        ...
        """Handle requests with direct route specified."""
        
        logger.info(f"Using direct route: {agent_url}")
    ...
    async def _send_agent_request(
        ...
        try:
            ...
            yield self._router_response(f"Sending user's query to agent...", "", False, agent_url)
    ...
    async def _get_agent_url(self, agent_cards: List[Dict[str, str]], agent_name: str) -> Optional[str]:
